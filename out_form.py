...
def SaveGraphicInPNG(list):
    excel = win32.gencache.EnsureDispatch('Excel.Application')
    workbook = excel.Workbooks.Open('C:/Users/delat/PycharmProjects/neural/графики пациентов/' + f'{list}')
    worksheet = workbook.Worksheets('grafik')
    c = 1
    for chart in worksheet.ChartObjects():
        file_name = f'{list}'+' '+f'{c}.png'
        ch = worksheet.ChartObjects(chart.Name).Chart
        ch.Export('C:/Users/delat/PycharmProjects/neural/Графики/' + f'{file_name}', 'PNG')
        print(chart.Name)
        c += 1
    workbook.Close()
    excel.Quit()
def createWordFile(rows,list):
    surname = rows[2]
    date = rows[4]
    doc = Document()

    interferon = float(rows[30])/float(rows[31])
    fno = float(rows[32]) / float(rows[33])
    ...
    picture1 = doc.add_picture('C:/Users/delat/PycharmProjects/neural/Графики/'+list+' 1.png', width=Inches(5))
    doc.add_paragraph('График B-клеточного звена')
    picture2 = doc.add_picture('C:/Users/delat/PycharmProjects/neural/Графики/'+list+' 2.png', width=Inches(5))
    doc.add_paragraph('График цитокиновых пар')
    #picture3 = doc.add_picture('C:/Users/delat/PycharmProjects/neural/Графики/'+list+' 3.png', width=Inches(5))
    print("ааа")

    if date == 'None':
        date = ''

    doc.save('C:/Users/delat/PycharmProjects/neural/Печатные Формы/' + f'{surname}{date}.docx')
...
def createRecomendation(rows):
    ...
    interferon = round (float(rows[30]) / float(rows[31])  ,2)
    fno = round (float(rows[33]) / float(rows[32])  ,2)
    interlikin = round (float(rows[34]) / float(rows[35])  ,2)
...
def Graphic(rows,list):
    ...
    wb = Workbook()
    ws = wb.active
    curr_wb = load_workbook('C:/Users/delat/PycharmProjects/neural/графики пациентов/' + f'{list}')
    ws = curr_wb['grafik']
    ...
    curr_wb.save('C:/Users/delat/PycharmProjects/neural/графики пациентов/' + f'{list}')
    curr_wb.close()

file_names = [
    'Абрамов 2004.09.24.xlsx',
    'Абрамов 2005.01.17.xlsx',
    'Баринов 2009.10.14.xlsx',
    'Баринов 2010.04.14.xlsx',
    'Бородина None.xlsx',
    'Дылдин 40303.xlsx',
    'ЗУБОВ None.xlsx',
    'ЗУЕВА 1999.05.05.xlsx',
    'Корзухина 40239.xlsx',
    'Немцев None.xlsx',
    'Сорокина 2009.06.05.xlsx',
    'Тарасова 2010.06.05.xlsx',
    'Тиккоева 2004.11.15.xlsx',
    'Тиккоева 2005.11.11.xlsx',
    'Тиккоева 2006.10.27.xlsx',
    'Тиккоева 2007.12.12.xlsx',
    'Чеснокова 2007.06.29.xlsx',
    'Чеснокова 2007.08.29.xlsx',
    'Чеснокова 2008.01.18.xlsx'
]

for file_name in file_names:
    SaveGraphicInPNG(file_name)

 #createWordFile(result, list)
 # Graphic(result, file_name)
#createRecomendation(result)
cursor.close()
conn.close()
